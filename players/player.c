...
//const char deployment[]="Bd0e0f0g0 Ce2f2g2 Ci2i3i4 Db2c2 Df4g4 Dh6i6 Sd4 Sc6 Sf6 Sg8 ";
//const char deployment[]="Bf5f6f7f8 Cc0d0e0 Cg1g2g3 Dc8d8 Dh5i5 Di2i3 Sb3 Sb5 Sd4 Sh7 ";
...
void init_board(void)
{
 int ix, iy;
  ...
  for(int i=0;i<9;i++){
     if(i%2==0)init[i][0]=abs((9-i)*i)*2;
      if(i%2==0)init[i][8]=abs((9-i)*i)*2;
       if(i%2==0)init[0][i]=abs((9-i)*i)*2;
        if(i%2==0)init[8][i]=abs((9-i)*i)*2;
  }
...
void respond_with_shot(void)
{
  ...
  int best=0;
   ...
   for(int j=0;j<81;j++){
    ...
    int nx=i/9;
   int ny=i%9;
   if(size== 4 && (nx-ny+9)%size!=0)continue;
   if(size== 3 && (nx-ny+9)%size!=2)continue;
   if(size== 2 && (nx-ny+9)%size!=0)continue;
   if(enemy_board[nx][ny]!=UNKNOWN)continue;
   if(inside(nx,ny)==1){
   x=nx;
   y=ny;
...
for(int x=0;x<9;x++){
for(int y=0;y<9;y++){ 
if(enemy_board[x][y]==BSHIP){
  ship_b(x,y);
  counter[4]++;
}
else if(enemy_board[x][y]==CSHIP){
  ship_c(x,y);
  counter[3]++;
}
else if(enemy_board[x][y]==DSHIP){
  ship_d(x,y);
  counter[2]++;
}
else if(enemy_board[x][y]==SSHIP){
  ship_s(x,y);
  counter[1]++;
}
}
