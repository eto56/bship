...
void respond_with_shot(void)
{
  ...
  int best=0;
   ...
   break;
    ...
    if(enemy_board[3][i]==UNKNOWN){
   x=3;
   y=i;
   break;
   }
    if(enemy_board[i][3]==UNKNOWN){
   x=i;
   y=3;
   break;
   }
   if(enemy_board[6][i]==UNKNOWN){
   x=6;
   y=i;
   break;
   }
    if(enemy_board[i][6]==UNKNOWN){
   x=i;
   y=6;
   break;
   }
...
if(counter[1]==4)size=2;
else if(counter[2]==6)size=3;
else if(counter[3]==6)size=4;
else if(counter[4]==4)size=5;
else size=1;
}
